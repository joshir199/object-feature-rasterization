...
renderCUDA(
	...
	const float* __restrict__ object_feature,
	...
	float* __restrict__ dL_dobject_feature,
...
)
{
	...
	__shared__ float collected_object_feature[NUM_OBJECT_FEATURE * BLOCK_SIZE];
	...
	float accum_rec[C] = { 0 };
	float accum_rec_feature[NUM_OBJECT_FEATURE] = { 0 };
	float dL_dpixel[C];
	float accum_depth_rec = 0;
	float dL_dpixel_depth;
	float dL_dpixel_feature[NUM_OBJECT_FEATURE];
	float accum_alpha_rec = 0;
	float dL_dalpha;
	if (inside) {
		for (int i = 0; i < C; i++)
			dL_dpixel[i] = dL_dpixels[i * H * W + pix_id];
		for (int i = 0; i < NUM_OBJECT_FEATURE; i++)
			dL_dpixel_feature[i] = dL_dpixels_feature[i * H * W + pix_id];
	...
	float last_feature[NUM_OBJECT_FEATURE] = { 0 };
	float last_depth = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < NUM_OBJECT_FEATURE; i++)
				collected_object_feature[i * BLOCK_SIZE + block.thread_rank()] = object_feature[coll_id * NUM_OBJECT_FEATURE + i];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < NUM_OBJECT_FEATURE; ch++)
			{
				const float f = collected_object_feature[ch * BLOCK_SIZE + j];
				...
				const float dL_dfeaturechannel = dL_dpixel_feature[ch];
				/**************************************************************************************************/
				dL_dalpha += (f - accum_rec_feature[ch]) * dL_dfeaturechannel; // Only works for semnatic-meaning feature. Disable this line for general features.
				/**************************************************************************************************/

				// Update the gradients w.r.t. semnatic feature of the Gaussian.
				// Atomic, since this pixel is just one of potentially
				// many that were affected by this Gaussian.
				atomicAdd(&(dL_dobject_feature[global_id * NUM_OBJECT_FEATURE + ch]), dchannel_dcolor * dL_dfeaturechannel);
			}
